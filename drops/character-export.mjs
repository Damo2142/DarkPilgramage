/**
 * character-export.mjs — Export PC actors from Foundry to Co-DM
 * Place at: ~/foundry-data/Data/modules/dark-pilgrimage/scripts/character-export.mjs
 */
...
function mapActor(actor) {
  ...
  for (const [abbr, fullName] of Object.entries(SKILL_NAMES)) {
    const statMod = Math.floor(((abils[SKILL_ABILITY[abbr]]?.value || 10) - 10) / 2);
    const val = sys.skills?.[abbr]?.value || 0;
    skills[fullName] = {
      modifier: statMod + (val === 2 ? profBonus * 2 : val === 1 ? profBonus : 0),
  ...
  return {
    ...
    level: totalLevel,
    race: actor.items.find(i => i.type === 'race')?.name || sys.details?.race || 'Unknown',
...
async function exportCharacters() {
  ...
  if (!pcs.length) { ui.notifications.warn('No PC actors found.'); return; }
...
Hooks.on('renderActorDirectory', (app, html, data) => {
  ...
  btn.className = 'dp-export-btn';
  btn.innerHTML = '⛧ Export Characters → Co-DM';
  btn.style.cssText = 'margin:4px 8px;width:calc(100% - 16px);background:#1a1a22;border:1px solid #8a7030;color:#c9a54e;font-size:11px;padding:4px 8px;cursor:pointer;border-radius:3px;';
  btn.addEventListener('click', exportCharacters);

  // html may be jQuery or plain element in v13
...
Hooks.once('ready', () => {
  ...
  console.log('[Dark Pilgrimage] Character export ready');
});
